...
def create_patients_table(cur, conn):
   ...
   cur.execute("CREATE TABLE \"Patients\"(\"pet_id\" INTEGER PRIMARY KEY, \"name\" TEXT, \"species_id\"  NUMBER, \"age\" INETGER, \"cuteness\" INTEGER, \"aggressiveness\" NUMBER)")
   conn.commit()

# ADD FLUFFLE TO THE TABLE
def add_fluffle(cur, conn):
    cur.execute("INSERT INTO Patients (pet_id, name, species_id, age, cuteness, aggressiveness) VALUES(?, ?, ?, ?, ?, ?)", (0, 'Fluffle', 'Rabbit', 0, 3, 90, 100))
    conn.commit()
...
def add_pets_from_json(filename, cur, conn):
    ...
    for item in json_data:
        ...
        cur.execute("INSERT INTO Patients (pet_id, name, species_id, age, cuteness, aggressiveness) VALUES(?,?,?,?,?,?)",(pet_id, name, species_id, age, cuteness, aggressiveness)) #use question mark
